...
const DashboardHistory = () => {
  ...
  const handleDownload = (e, id) => {
    e.stopPropagation();
    // Implementation for downloading report could go here
    console.log("Download report for:", id);
    alert("Report download functionality is currently being processed.");
  };
  ...
  return (
    <MatrixContext>
      <main className="pb-24 px-6 md:px-12 text-slate-900 dark:text-white font-sans transition-colors duration-300 pt-20">
        <div className="px-6 md:px-12 transition-colors duration-300">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end md:items-center max-w-6xl mx-auto mb-8 mt-12 gap-4">
          <div>
            <h1 className="text-3xl font-bold mb-2 flex items-center gap-2 text-slate-900 dark:text-white">
              <ClipboardDocumentListIcon className="w-8 h-8 text-blue-600 dark:text-blue-500" />
              Assessment History
            </h1>
          ...
          <button
            ...
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-medium transition shadow-lg shadow-blue-600/20 dark:shadow-blue-900/20"
          >
        ...
        <div className="bg-white/80 dark:bg-[#16181c]/80 backdrop-blur-sm rounded-xl border border-slate-200 dark:border-gray-800 overflow-hidden max-w-6xl mx-auto shadow-sm dark:shadow-xl transition-colors duration-300">
          {/* --- TABLE HEADER --- */}
          <div className="grid grid-cols-12 gap-6 p-4 border-b border-slate-200 dark:border-gray-800 text-slate-500 text-xs font-bold uppercase tracking-wider bg-slate-50/50 dark:bg-[#1c1f24]/50">
          ...
          {assessments.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-16 text-center">
              <div className="w-20 h-20 bg-slate-100 dark:bg-gray-800/50 rounded-full flex items-center justify-center mb-6">
                <ClipboardDocumentListIcon className="w-10 h-10 text-slate-400 dark:text-gray-600" />
              </div>
              ...
              <button
                onClick={() => router.push("/assessments/SecurityAssessment")}
                className="flex items-center gap-2 px-6 py-2 border border-blue-600 text-blue-600 dark:text-blue-400 rounded-full hover:bg-blue-600 hover:text-white transition"
              >
            ...
            assessments.map((item, index) => (
              <div
                key={index}
                className={`grid grid-cols-12 gap-6 p-4 border-b border-slate-200 dark:border-gray-800 items-center hover:bg-slate-50 dark:hover:bg-[#1c1f24] transition group ${selectedId === item._id
                  ? "bg-blue-50 dark:bg-blue-900/10 border-l-4 border-l-blue-600 dark:border-l-blue-500"
                  : ""
                  }`}
              >
                {/* Date */}
                <div className="col-span-3 text-slate-600 dark:text-gray-300 text-sm flex items-center gap-3">
                  <div className="p-2 bg-slate-100 dark:bg-gray-800 rounded-lg text-slate-500 dark:text-gray-400">
                ...
                {/* Score */}
                <div className="col-span-2 font-bold text-xl text-slate-900 dark:text-white text-center">
                  {item.score}
                </div>
